        ...
        case 'OrderAppFinish':
            ...
            foreach ($_SESSION['oderCreate']['item'] as $ItemId => $ItemAmount):
                ...
                if (!$Read->getResult()):
                    ...
                    if ($pdt_dimension_width > 105 || $pdt_dimension_depth > 105 || $pdt_dimension_heigth > 105):
                        $xl = 1;
                    endif;
                endif;
            endforeach;
                ...
                if (!array_key_exists('shipcode', $CorreiosRetorno)):
                    foreach ($CorreiosRetorno as $modalidade):
                        if ($modalidade['status'] === 'OK'):
                            $jSON['cart_shipment'] .= "<li><label><input type='radio' name='shipment' value='" . $modalidade['valorfrete'] . "' id='{$modalidade['shipcode']}'><span>" . getShipmentTag(intval($modalidade['shipcode'])) . " - R$ " . number_format($modalidade['valorfrete'], '2', ',', '') . " - {$modalidade['prazoentrega']} dias úteis</span></label></li>";
                        endif;
                    endforeach;
                else:
                    if ($CorreiosRetorno['status'] === 'OK'):
                        $jSON['cart_shipment'] .= "<li><label><input type='radio' name='shipment' value='" . $CorreiosRetorno['valorfrete'] . "' id='{$CorreiosRetorno['shipcode']}'><span>" . getShipmentTag(intval($CorreiosRetorno['shipcode'])) . "  - R$ " . number_format($CorreiosRetorno['valorfrete'], '2', ',', '') . " - {$CorreiosRetorno['prazoentrega']} dias úteis</span></label></li>";
                    endif;
                endif;
            endif;
