...
if (!empty($postback)) {
    ...
    $PaymentOrderCode = $postback['transaction']['id'];
    $PaymentStatus = $postback['transaction']['status'];

    $Read->ExeRead(DB_ORDERS, "WHERE order_code = :orid", "orid={$PaymentOrderCode}");
    if ($Read->getResult()):
        ...
        if ($PaymentStatus == 'waiting_payment'):
            ...
            if ($Read->getResult()):
                ...
                foreach ($Read->getResult() as $Item):
                    ...
                    //$BodyMail .= "<tr><td style='border-bottom: 1px solid #cccccc; padding: 10px 0 10px 0;'>" . str_pad($i, 5, 0, STR_PAD_LEFT) . " - " . Check::Words($Item['item_name'], 5) . "{$ProductSize}</td><td style='border-bottom: 1px solid #cccccc; padding: 10px 0 10px 0; text-align: right;'>R$ " . number_format($Item['item_price'], '2', ',', '.') . " * <b>" . str_replace('.', ',', (float) $Item['item_amount']) . " {$pdtUnity}</b></td><td style='border-bottom: 1px solid #cccccc; padding: 10px 0 10px 0; text-align: right;'>R$ " . number_format($Item['item_amount'] * $Item['item_price'], '2', ',', '.') . "</td></tr>";
                endforeach;
        ...
        elseif ($PaymentStatus == 'paid'):
            ...
            $NotifyMail .= "Código Pagar.me: {$order_code}<br><br>";
            ...
            $Email->EnviarMontando("[#{$order_id}] Pagamento Aprovado!", $Notify, SITE_NAME, MAIL_USER, MAIL_SENDER, SITE_ADDR_EMAIL);
        ...
        elseif ($PaymentStatus == 5):
            ...
            $NotifyMail .= "E-mail: {$Client['user_email']}<br>";
            $NotifyMail .= "</p>";
            $NotifyMail .= "<p><b>Dica:</b> Ligue para o cliente para resolver o caso. Se não conseguir, acesse sua conta Pagar.me e envie os comprovantes de entrega do mesmo!</p>";
            $NotifyMail .= "<p><b>Importante:</b> Pedidos com disputas que não são resolvidas, são devolvidos pela Pagar.me. Não deixe de resolver para não perder o pagamento!</p>";
            $NotifyMail .= "<p><i>Atenciosamente " . ADMIN_NAME . "!</i></p>";
            $Notify = str_replace('#mail_body#', $NotifyMail, $MailContent);
            $Email->EnviarMontando("[#{$order_id}] Pagamento em disputa!", $Notify, SITE_NAME, MAIL_USER, MAIL_SENDER, SITE_ADDR_EMAIL);
        ...
        elseif ($PaymentStatus == 'refunded'):
            ...
            $NotifyMail .= "<p><i>Atenciosamente " . ADMIN_NAME . "!</i></p>";
            $Notify = str_replace('#mail_body#', $NotifyMail, $MailContent);
            $Email->EnviarMontando("[#{$order_id}] Pedido cancelado!", $Notify, SITE_NAME, MAIL_USER, MAIL_SENDER, SITE_ADDR_EMAIL);

        elseif ($PaymentStatus == 'refused'):
            /**
             * CANCELADO
             */
            $BodyMail = "<p style='font-size: 1.2em;'>Caro(a) {$Client['user_name']},</p>";
