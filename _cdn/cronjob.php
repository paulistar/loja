        ...
        foreach ($Read->getResult() as $OrderCancel):
            ...
            if ($Read->getResult()):
                ...
                foreach ($Read->getResult() as $Item):
                    $Read->FullRead("SELECT a.attr_size_code FROM " . DB_PDT_STOCK . " s INNER JOIN " . DB_PDT_ATTR_SIZES . " a ON s.size_id = a.attr_size_id WHERE s.stock_id = :id", "id={$Item['stock_id']}");
                    $ProductSize = ($Read->getResult() && !empty($Read->getResult()[0]['attr_size_code']) ? " ({$Read->getResult()[0]['attr_size_code']})" : null);
