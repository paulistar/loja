...
switch ($Action):
    ...
    case 'address':
        ...
        $Read->LinkResult(DB_USERS_ADDR, 'addr_id', $address, 'addr_id');
        if (!$Read->getResult()):
            $jSON['trigger'] = AjaxErro("<b>Oppsss:</b> {$_SESSION['userLogin']['user_name']}, esse endereço não existe ou foi removido recentemente!", E_USER_WARNING);
        else:
            $Update->ExeUpdate(DB_USERS_ADDR, $POST, "WHERE addr_id = :id", "id={$address}");
            $jSON['trigger'] = AjaxErro("<b>TUDO CERTO:</b> Olá {$_SESSION['userLogin']['user_name']}, seu endereço foi atualizado com sucesso!");
        endif;
        break;
endswitch;
