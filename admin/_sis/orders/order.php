                ...
                <blockquote>
                    ...
                    if ($Read->getResult()):
                        ...
                        foreach ($Read->getResult() as $Items):
                            ...
                            if ($Read->getResult()):
                                echo "[&nbsp;&nbsp;] {$Items['item_amount']} - ({$Read->getResult()[0]['pdt_code']}) {$Items['item_name']}{$PdtVariation}<br>";
                            else:
                                echo "[&nbsp;&nbsp;] {$Items['item_amount']} - {$Items['item_name']}{$PdtVariation}<br>";
                            endif;
                        endforeach;
                ...
                <div class="single_order_items">
                    ...
                    if (!$Read->getResult()):
                        ...
                        foreach ($Read->getResult() as $Item):
                            ...
                            if ($Item['pdt_id']):
                                echo "<p><b>" . str_pad($Item['item_id'], 5, 0, STR_PAD_LEFT) . "</b> - <a target='_blank' href='dashboard.php?wc=products/create&id={$Item['pdt_id']}' title='Ver Produto!'>" . Check::Words($Item['item_name'], 3) . "{$PdtVariation}</a></p>";
                            ...
                            echo "<p>R$ " . number_format($Item['item_price'], '2', ',', '.') . " * <b>" . str_replace('.', ',', (float) $Item['item_amount']) . " {$pdtUnity}</b></p>";
