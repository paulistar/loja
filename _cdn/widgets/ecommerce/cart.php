...
if (!empty($_SESSION['redirect'])):
    ...
    echo "<span class='redirect_name' data-name='{$_SESSION['userLogin']['user_name']}'></span>";
    if (!empty($_SESSION['wc_cupom'])):
        echo "<span class='redirect_coupon' data-coupon='{$_SESSION['wc_cupom']}'></span>";
    endif;
    echo "</div>";
    ...
    if ($CartAction == 'home'):
        ...
        foreach ($_SESSION['wc_order'] as $ItemId => $ItemAmount):
            ...
            if ($Read->getResult()):
                ...
                /* CUSTOM BY ALISSON */
                echo "<p class='item'><a href='" . BASE . "/produto/{$pdt_name}' title='Ver detalhes de {$pdt_title}'>{$pdt_title}</a>{$PdtVariation}</p>";
                ...
                echo "<p><button id='{$ItemId}' class='change wc_cart_change_less'>-</button><input id='{$ItemId}' class='wc_cart_change' type='text' value='{$ItemAmount}' step='{$pdt_step}' max='{$StockInventory}'><button id='{$ItemId}' class='change wc_cart_change_plus'>+</button>" . ($pdt_unity ? "<span class='unity'>{$pdt_unity}</span>" : '') . "</p>";
    ...
    elseif ($CartAction == 'pagamento'):
        ...
        elseif (!$Read->getResult()):
            ...
            if ($order_status == 1 || $order_status == 6 || date('Y-m-d H:i:s', strtotime($order_date . "+" . E_ORDER_DAYS . "days")) < date('Y-m-d H:i:s')):
                ...
                echo "<div class='workcontrol_order'>";
                echo "<div class='workcontrol_order_forms'>";
                if (METHOD_PAYMENT == 'pagseguro'):
                    require 'PagSeguroWc/Payment.workcontrol.php';
                elseif (METHOD_PAYMENT == 'pagarme'):
                    require 'PagarmeWc/Payment.workcontrol.php';
                elseif (METHOD_PAYMENT == 'cielo'):
                    require 'CieloWc/Payment.workcontrol.php';
                elseif (METHOD_PAYMENT == 'moip'):
                    require 'Moip/Payment.workcontrol.php';
                else:
                    Erro("Opss ainda não configuramos nenhum método de pagamento. Volte em breve!", E_USER_NOTICE);
                endif;
                echo "</div>";
                ...
                if ($Read->getResult()):
                    ...
                    foreach ($Read->getResult() as $SideProduct):
                        ...
                        if ($SideProduct['pdt_id']):
                            ...
                            /* CUSTOM BY ALISSON */
                            echo "<p class='product'><img title='{$SideProduct['item_name']}' alt='{$SideProduct['item_name']}' src='" . BASE . "/tim.php?src=uploads/{$Read->getResult()[0]['pdt_cover']}&w=" . THUMB_W / 5 . "&h=" . THUMB_H / 5 . "'/><span>" . Check::Chars($SideProduct['item_name'], 42) . "{$PdtVariation}</span></p>";
