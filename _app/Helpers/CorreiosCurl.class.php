...
class CorreiosCurl
{
    ...
    {
        return array(
            'nCdEmpresa' => $this->nCdEmpresa ? $this->nCdEmpresa : '08082650',
            'sDsSenha' => $this->sDsSenha ? $this->sDsSenha : '564321',
    ...
    {
        try {
            $resultado = Utils::CurlExec('http://ws.correios.com.br/calculador/CalcPrecoPrazo.aspx?' . http_build_query($this->buildQuoteRequest()), [], []);
    ...
    private function parseQuoteResult($result)
    {
        if ($result['status'] === 200) {
            ...
            foreach ($resultado->cServico as $row):
                $this->vlTotalFrete = number_format(str_replace(',', '.', str_replace('.', '', (string)trim($row->Valor))), 2, '.', '');
                ...
                $this->prazoEntrega = (string)trim($row->PrazoEntrega);
                $this->prazoEntrega = $this->prazoEntrega + $this->shipmentDelay;

                if (floatval(number_format(floatval(str_replace(',', '.', str_replace('.', '', $row->Valor))), 2, '.', ''))) {
                    $this->Result[] = array(
                        'status' => 'OK',
                        'shipcode' => (string)trim($row->Codigo),
                        'valorfrete' => $this->vlTotalFrete,
                        'prazoentrega' => $this->prazoEntrega,
                        'ValorMaoPropria' => number_format(str_replace(',', '.', str_replace('.', '', (string)trim($row->ValorMaoPropria))), 2, '.', ''),
                        'ValorAvisoRecebimento' => number_format(str_replace(',', '.', str_replace('.', '', (string)trim($row->ValorAvisoRecebimento))), 2, '.', ''),
                        'ValorValorDeclarado' => number_format(str_replace(',', '.', str_replace('.', '', (string)trim($row->ValorValorDeclarado))), 2, '.', ''),
                        'EntregaDomiciliar' => (string)trim($row->EntregaDomiciliar),
                        'EntregaSabado' => (string)trim($row->EntregaSabado),
                        'Erro' => (string)trim($row->Erro),
                        'MsgErro' => (string)trim($row->MsgErro),
                        'ValorSemAdicionais' => number_format(str_replace(',', '.', str_replace('.', '', (string)trim($row->ValorSemAdicionais))), 2, '.', ''),
                        'obsFim' => (string)trim($row->obsFim)
                    );
                } else {
                    $this->Result[] = array(
                        'status' => 'ERROR',
                        'errorMsg' => (string)trim($row->MsgErro),
                        'shipcode' => (string)trim($row->Codigo),
                        'valorfrete' => $this->vlTotalFrete,
                        'prazoentrega' => $this->prazoEntrega,
                        'ValorMaoPropria' => number_format(str_replace(',', '.', str_replace('.', '', (string)trim($row->ValorMaoPropria))), 2, '.', ''),
