...
<header class="dashboard_header">
    <div class="dashboard_header_title">
        <h1 class="icon-images">Banners</h1>
...
<div class="dashboard_content">
    ...
    $Read->ExeRead(DB_BANNERS, "WHERE banner_status = :status ORDER BY banner_date DESC LIMIT :limit OFFSET :offset", "status=1&limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
    if (!$Read->getResult()):
        $Pager->ReturnPage();
        ...
        $Pager->ExePaginator(DB_BANNERS, "WHERE banner_status = :status", "status=1");
